...
#         return (
#             val == "N/A" or
#             "PrintShare" in val or
#             "Next Section" in val or
#             len(val) > 2000
#         )
...
#             link_elements = soup.select(deep_cfg["selector"])
#             for el in link_elements:
#                 href = el.get(deep_cfg.get("attribute", "href"))
#                 if href and href.startswith("http"):
#                     sub_urls.add(href)
#                 elif href:
#                     sub_urls.add(requests.compat.urljoin(url, href))
#             print(f"🔗 Found {len(sub_urls)} subpages to crawl...\n")

#         elif isinstance(deep_cfg, list):
#             sub_urls.update(deep_cfg)
#             print(f"🔗 Using manually listed deep_links: {len(sub_urls)} URLs\n")
...
#             soup = BeautifulSoup(response.text, "html.parser")
#         except Exception as e:
#             print(f"❌ Failed to fetch subpage: {page_url} | Error: {e}")
#             return

#         data = {
#             "title": self.extract_text(soup, self.config["selectors"].get("title")) or "N/A",
#             "url": page_url,
#             "funding_amount": self.extract_text(soup, self.config["selectors"].get("funding")) or "N/A",
#             "deadline": self.extract_deadline(soup) or "N/A",
#             "program_type": self.config.get("program_type", "Incentive"),
#             "eligibility": self.extract_text(soup, self.config["selectors"].get("eligibility")) or "N/A"
#         }
...
#         raw = self.extract_text(soup, self.config["selectors"].get("deadline"))
#         if not raw:
#             return None
#         try:
...
#         self.pdf_min_size = self.config.get("pdf_min_size", 10_000)      # 10 KB
#         self.pdf_max_size = self.config.get("pdf_max_size", 10_000_000)  # 10 MB
...
#         data = {
#             "title": self.extract_text(soup, s.get("title")) or "N/A",
#             "url": url,
...
#             print("🤖 Falling back to AI extraction due to junk/missing fields...")
#             raw = soup.get_text(" ")
#             ai_result = self.ai.extract_fields(raw)
#             for k in ["funding_amount", "deadline", "eligibility"]:
#                 if self.is_bad(data[k]) and ai_result.get(k):
